...
for i in range(city_count):
    for j in range(city_count):
        Distance[i][j] = abs(data.iat[i, 0] - data.iat[j, 0]) + abs(data.iat[i, 1] - data.iat[j, 1])
...
for index in result_path:
    X.append(data.iat[index, 0])
    Y.append(data.iat[index, 1])
X.append(X[0])
Y.append(Y[0])
print(X)
print(Y)
...
fig = plt.figure()
ax2 = fig.add_subplot()
ax2.set_title('travel')
for i in range(len(x)):
    plt.annotate(result_path[i], xy=(X[i], Y[i]))
